...
export const initPayment = async (user: TUser, order: RentalOrder) => {
  ...
  try {
    const data = {
      ...
      currency: 'BDT',
      tran_id: transId,
      success_url: `${config.ssl.successUrl}?orderId=${order.id}&tranId=${transId}&status=success`,
      fail_url: `${config.ssl.failUrl}?orderId=${order.id}&tranId=${transId}&status=fail`,
      cancel_url: `${config.ssl.cancelUrl}?orderId=${order.id}&tranId=${transId}&status=cancel`,
